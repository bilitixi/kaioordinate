...
namespace Kaioordinate
{
    public partial class locationReportForm : Form
    {
        ...
        private void PrintDoc_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            if (currentRow == 0)
            {
                string header = "List of Location";
            ...
            while (currentRow < table.Rows.Count)
            {
                ...
                // Location ID
                e.Graphics.DrawString("Location ID:", boldfont, Brushes.Black, leftMargin, y);
                e.Graphics.DrawString(row["LocationID"].ToString(), font, Brushes.Black, leftMargin + 100, y);
                y += lineHeight;

                // Location Name
                e.Graphics.DrawString("Location Name:", boldfont, Brushes.Black, leftMargin, y);
                e.Graphics.DrawString(row["LocationName"].ToString() , font, Brushes.Black, leftMargin + 150, y);
                y += lineHeight;

                // Address
                e.Graphics.DrawString("Address:", boldfont, Brushes.Black, leftMargin, y);
                e.Graphics.DrawString(row["Address"].ToString(), font, Brushes.Black, leftMargin + 150, y);
                y += lineHeight;
                // Separator line
                e.Graphics.DrawLine(Pens.Black, leftMargin, y, e.MarginBounds.Right, y);
                y += lineHeight;
